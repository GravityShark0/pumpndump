...
(defun calculate-pomodoro-time (input)
  ...
  (let ((mins 0))
    ...
    (let* ((current-time (current-time))        ; Get the current time
           (time-delta (* mins 60))          ; Convert minutes to seconds
      ...
      (print (format "%i Pomodoro(s) is %s long and it would end at %s" input longing (format-time-string "%l:%M %p" future-time))))))

;; (message "You entered: %i" input))
